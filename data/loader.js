...
(async function() {
    ...
    // Função para carregar o script (geral)
    ...
    // Carregar o gamepad.js imediatamente
    await loadScript("gamepad.js");

    // Carregar os outros scripts de emulação
    async function loadOtherScripts() {
        for (let i = 0; i < scripts.length; i++) {
            await loadScript(scripts[i]);
        }
    }

    // Tente carregar o emulador e outros scripts
    try {
        if ('undefined' != typeof EJS_DEBUG_XX && true === EJS_DEBUG_XX) {
            await loadOtherScripts();
            await loadScript('emulator.css');
        } else {
            await loadScript('emulator.min.js');
            await loadScript('emulator.min.css');
        }
    } catch (e) {
        console.error("Erro ao carregar emulador ou scripts.", e);
    }

    // Configurações do emulador (não alteradas)
    const config = {};
    ...
    // Carregar idioma se necessário (não alterado)
    let systemLang;
    try {
