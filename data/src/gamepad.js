        ...
        gamepads.forEach((gamepad, index) => {
            ...
            this.gamepads.forEach((oldGamepad, oldIndex) => {
                ...
                oldGamepad.axes.forEach((axis, axisIndex) => {
                    ...
                    if (newVal !== val) {
                        ...
                        this.dispatchEvent('axischanged', {
                            gamepadId: `${gamepad.id}_${gamepad.index}`,
                            axis: axis,
                            value: newVal,
                            index: gamepad.index,
                ...
                gamepad.buttons.forEach((button, buttonIndex) => {
                    ...
                    if (pressed !== pressed2) {
                        if (pressed2) {
                            this.dispatchEvent('buttondown', {index: buttonIndex, label: this.getButtonLabel(buttonIndex), gamepadIndex: gamepad.index, gamepadId: `${gamepad.id}_${gamepad.index}`});
                        } else {
                            this.dispatchEvent('buttonup', {index: buttonIndex, label:this.getButtonLabel(buttonIndex), gamepadIndex: gamepad.index, gamepadId: `${gamepad.id}_${gamepad.index}`});
                        }
                    }
            ...
            if (!hasGamepad) {
                const uniqueId = `${gamepad.id}_${gamepad.index}`;
gamepads[index].__uniqueId = uniqueId;
this.gamepads.push(gamepads[index]);
                this.dispatchEvent('connected', {gamepadIndex: gamepad.index, gamepadId: `${gamepad.id}_${gamepad.index}`});
            }
        });
        ...
        for (let j=0; j<this.gamepads.length; j++) {
            ...
            if (!has) {
                this.dispatchEvent('disconnected', {gamepadIndex: this.gamepads[j].index, gamepadId: `${this.gamepads[j].id}_${this.gamepads[j].index}`});
